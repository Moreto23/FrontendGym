...
export class PagosComponent {
  metodo: 'YAPE' | 'PAYPAL' | 'MERCADO_PAGO' | '' = 'MERCADO_PAGO' as any;
  ...
  iniciar() {
    ...
    this.isPaying = true;
    this.pagos.iniciar(userId, this.metodo as any).subscribe({
      next: (res) => {
        this.pedidoId = res?.pedidoId ?? null;
        this.pagoId = res?.pagoId ?? null;
        this.monto = res?.monto ?? null;
        this.estadoPago = res?.estadoPago ?? null;
        this.isPaying = false;
      },
      error: (err) => {
